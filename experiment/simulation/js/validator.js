...
export function isValid() {
  ...
  let variableSubcktList = [
    ...
    convertToLowerCase(latch1_instance.value.trim()),
    convertToLowerCase(latch2_instance.value.trim()),
    "vdd",
    "gnd",
    "wmin",
    "lmin",
  ...
  if (
    latch1_instance.value.trim()[0] != "x" &&
    latch1_instance.value.trim()[0] != "X"
  ) {
    let msg =
      "When instantiating a sub circuit, the name of the instance must always start with 'x' or 'X'";
  ...
  if (
    latch2_instance.value.trim()[0] != "x" &&
    latch2_instance.value.trim()[0] != "X"
  ) {
    let msg =
      "When instantiating a sub circuit, the name of the instance must always start with 'x' or 'X'";
...
export function printObsTable() {
  ...
  let latch1_in = convertToLowerCase(
    document.getElementById("latch1-input").value.trim()
  );
  let latch1_clk = convertToLowerCase(
    document.getElementById("latch1-clk").value.trim()
  );
  let latch1_out = convertToLowerCase(
    document.getElementById("latch1-output").value.trim()
  );
  let latch2_in = convertToLowerCase(
    document.getElementById("latch2-input").value.trim()
  );
  let latch2_clk = convertToLowerCase(
    document.getElementById("latch2-clk").value.trim()
  );
  let latch2_out = convertToLowerCase(
    document.getElementById("latch2-output").value.trim()
  );
  let gateCallIn1 = document.getElementById("gate-call-input1");
  let gateCallIn2 = document.getElementById("gate-call-input2");
  let gateCallOut = document.getElementById("gate-call-output");
      ...
      if (
        ...
        if (latch1_in === in1 && latch1_clk === in2) {
          ...
          if (
            ...
            latch1_out !== in2 &&
            latch1_out !== out &&
            regex.test(latch1_out)
          ) {
            {
              console.log("5");
              if (
                latch2_in === latch1_out &&
                latch2_clk === inv_out1 &&
                latch2_out === out
              ) {
                console.log("6");
          ...
          if (
            ...
            latch2_out !== in2 &&
            latch2_out !== out &&
            regex.test(latch2_out)
          ) {
            console.log("8");
            if (
              latch1_in === latch2_out &&
              latch1_clk === inv_out1 &&
              latch1_out === out
            ) {
              console.log("9");
  ...
  if (correct === true) {
    ...
    <div class="is-size-5" style="color: #3273dc; margin-bottom: 1em; margin-top: 1em;">
